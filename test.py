...
if __name__ == "__main__":
    ...
    labels = ["Lorentzian", "DRW", "Matern"]
    # You can also use Lorentzian from models.celerite_models (which is defined in terms of variance, Q and omega)
    kernel = Lorentzian(log_S0=log_variance_qpo, log_Q=np.log(Q), log_omega0=log_d) + RealTerm(log_a=np.log(variance_drw), log_c=np.log(w_bend)) + Matern32Term(log_sigma_matern, log_rho_matern, eps=1e-8)
    ...
    jitterterm = JitterTerm(np.log(np.sqrt(lc_variance)))
    ...
    #models = [realterm, lorentzian + realterm] # I don't care for Matern-3/2
    ...
    for kernel in models:
        ...
        gp.derive_posteriors(fit=True, max_steps=2000, walkers=2 * cpus, cores=cpus, progress=False) # MCMC is done in here
    ...
    plt.show()
    # 
    gp = best_gp # otherwise there are weird things being called, gp is only the last gp in the models list, not the best
